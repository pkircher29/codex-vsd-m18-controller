...
- Choose **Design with AI** to generate and preview an 18-key draft from a plain-language request. M18 Foundry can discover models from Ollama, OpenRouter, OpenAI, Anthropic, Gemini, or any standard OpenAI-compatible local/hosted endpoint (including LM Studio-style local servers and multi-provider gateways).
